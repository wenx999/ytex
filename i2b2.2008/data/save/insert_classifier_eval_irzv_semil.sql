/*
 * identical to insert_classifier_eval_irzv.sql except for verification queries.
 * with semi-supervised algorithms we do not duplicate instances in both the training
 * and test sets - they are only in the training sets.
 */
...
(
	/*
	 * best f1 score by experiment (hotspot cutoff) and semil parameters
	 */
	select label, param1, param2, distance, degree, gamma, soft_label, norm_laplace, avg(f1) f1
	from classifier_eval_irzv t
	inner join classifier_eval e on e.classifier_eval_id = t.classifier_eval_id
	inner join classifier_eval_semil l on e.classifier_eval_id = l.classifier_eval_id
	where experiment = 'bag-word-semil'
	and name = 'i2b2.2008'
	and run > 0
	and fold > 0
	group by label, param1, param2, distance, degree, gamma, soft_label, norm_laplace
) s
group by label
order by cast(label as decimal(2,0));