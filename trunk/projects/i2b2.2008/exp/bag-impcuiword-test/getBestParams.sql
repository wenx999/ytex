delete from i2b2_2008_cv_best where experiment = 'bag-impcuiword';
...
(
  select label, cost, param1, run, fold, avg(f1) f1
    ...
    and experiment in ('bag-impcuiword')
...
select label, 'bag-impcuiword', f1
from
(
	/*
...
inner join i2b2_2008_cv_best c 
    on c.label = s.label 
    and s.f1 >= c.f1
    and c.experiment = 'bag-impcuiword'
group by c.label
;
