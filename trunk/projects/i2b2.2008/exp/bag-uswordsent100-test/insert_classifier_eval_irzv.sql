...
inner join i2b2_2008_cv_best b on b.label = e.label and b.experiment = 'bag-uswordsent100'
set e.param1 = b.param1
where e.experiment = 'bag-uswordsent100-test' and e.name = 'i2b2.2008'
;
...
select disease_id label, ir_class_id, tp, tn, fp, fn
from
(
	/* truth table */
	select ir_class_id, disease_id, 
	  sum(case
	...
	from
	(
		select a.docId, ds.disease_id, ja.judgement_id ir_class_id, jauto.judgement_id pred_class_id, jgold.judgement_id target_class_id
	...
	group by ir_class_id, disease_id
) s;

create index IX_hotspot_zero_vector_tt on  tmp_hotspot_zero_vector_tt(label, ir_class_id);
...
(
    ...
    (
	    ...
	    select label, ir_class_id, tp+tn+fp+fn tot
...
where e.experiment = 'bag-uswordsent100-test'
...
(
	...
	(
		...
		inner join classifier_eval e 
			...
			and e.experiment = 'bag-uswordsent100-test'
			and e.name = 'i2b2.2008'
			and e.run = 0
			and e.fold = 0
    /* 
    ...
    */
    inner join i2b2_2008_disease d 
		...
		left join tmp_hotspot_zero_vector_tt z
			on (z.label, z.ir_class_id) = (e.label, ir.ir_class_id)	) s
) s
;
...
(
    ...
    (
		...
		and experiment = 'bag-uswordsent100-test'
	    and run = 0
	    and fold = 0
    ) s
...
(
	...
	where experiment = 'bag-uswordsent100-test'
	and name = 'i2b2.2008'
	and run = 0
	and fold = 0
