...
public class CharacterOffsetToLineTokenConverterCtakesImpl implements
	...
	protected Logger logger = Logger
			.getLogger(CharacterOffsetToLineTokenConverterCtakesImpl.class
					.getName());
	protected JCas jcas;
	...
	protected void buildSentenceBoundaryMap() {
	...
	protected void buildTokenBoundaryMap() {
	...
	protected Sentence findPreviousOrCurrentSentence(int characterOffset) {
	...
	protected int adjustOffsetToBestMatch(int original) {
		...
		if (newValue == null) {
			...
			if (original == newValue)
				...
				logger.finest("found previous token boundary. original: "
						+ original + "; new value: " + newValue);
		}
	...
	protected LineAndTokenPosition convertCharacterOffsetToLineToken(
			int characterOffset) {
		logger.finest("entering CharacterOffsetToLineTokenConverterCtakesImpl.convertCharacterOffsetToLineToken() with a characterOffset of: "
				+ characterOffset);
		...
		logger.finest("finding current or previous sentence for character offset "
				+ characterOffset);
		Sentence sentence = findPreviousOrCurrentSentence(characterOffset);
		if (sentence == null) {
			logger.info("current or previous sentence IS NULL!");
		} else {
			logger.finest("current or previous sentence -- id: "
					+ sentence.getAddress() + "; begin: " + sentence.getBegin()
					+ "; end: " + sentence.getEnd());
		}
		...
		// "First token in sentence not found!! (character offset request = "
		// + characterOffset);
		// }
	...
	// // jcas.getAnnotationIndex(sentenceTypeId);
	//
	// ConstraintConstructorFindContainedBy constraintConstructorFindContainedBy
	// = new ConstraintConstructorFindContainedBy(
	// jcas);
	...
	// e.setLine(lineNumber);
	// e.setTokenOffset(endTokenWordNumber);
	// System.out.println("Adding lineTokenEnding " + lineNumber
	// + " offset = " + endTokenWordNumber);
	// list.add(e);
	// return list;
	// }
