...
import ytex.uima.mapper.DateMapper;
...
public class DateAnnotator extends JCasAnnotator_ImplBase {
	...
	private ThreadLocal<SimpleDateFormat> tlDateFormat = new ThreadLocal<SimpleDateFormat>() {
		...
		protected SimpleDateFormat initialValue() {
			return new SimpleDateFormat(DateMapper.DATE_FORMAT);
		}
	};
	...
	public void process(JCas jCas) throws AnalysisEngineProcessException {
		AnnotationIndex annoIndex = jCas.getAnnotationIndex(jCas
				.getTypeSystem().getType(dateType));
		FSIterator iter = annoIndex.iterator();
		while (iter.hasNext()) {
