...
/**
 * fixed to handle newlines in sentences properly
 * 
 * @author vijay
 * 
 */
public class SingleDocumentProcessorCtakes extends SingleDocumentProcessor {
	...
	protected void preExecutionTest() {
		if (converter2 == null) {
			converter2 = new CharacterOffsetToLineTokenConverterCtakesImpl(jcas);
		}
	}
	...
	public ArrayList<ArrayList<String>> construct2DTokenArray(JCas jcas) {
		...
		AnnotationIndex<Annotation> sentenceAnnotationIndex = jcas
		...
		for (Annotation annotation : sentenceAnnotationIndex) {
			...
			AnnotationIndex<Annotation> tokenAnnotationIndex = jcas
					.getAnnotationIndex(BaseToken.type);
			FSIterator<Annotation> tokensInThisSentenceIterator = tokenAnnotationIndex
					.subiterator(sentence);
			ArrayList<String> arrayOfTokens = new ArrayList<String>();
			// for (Annotation baseTokenAnnotationUntyped :
			// tokenAnnotationIndex)
			while (tokensInThisSentenceIterator.hasNext()) {
				Annotation baseTokenAnnotationUntyped = tokensInThisSentenceIterator
						.next();
				// // ignore tokens that are outside of the sentence.
				// // there has to be a better way to do this with Constraints,
				// but this
				// // should work for now...
	...
	public LineAndTokenPosition convertCharacterOffsetToLineToken(
			int characterOffset) {
		return converter2.convert(characterOffset);
		// int baseTokenTypeId = BaseToken.type;
		//
		// ConstraintConstructorFindContainedBy
		// constraintConstructorFindContainedBy = new
		// ConstraintConstructorFindContainedBy(jcas);
		// ConstraintConstructorFindContainedWithin
		// constraintConstructorFindContainedWithin = new
		// ConstraintConstructorFindContainedWithin(jcas);
		//
		// Type sentenceType =
		// jcas.getTypeSystem().getType(Sentence.class.getName());
		// Type baseTokenType =
		// jcas.getTypeSystem().getType(BaseToken.class.getName());
		//
		...
		// {
		// throw new
		// RuntimeException("Surrounding sentence annotation not found!!");
		// }
	...
	public List<LineAndTokenPosition> calculateBeginAndEndOfConcept(
			ApiConcept problem) {
		return calculateBeginAndEndOfConcept(problem.getBegin(),
				problem.getEnd());
	}
	...
	public List<LineAndTokenPosition> calculateBeginAndEndOfConcept(
		...
		//
		//
		// ArrayList<LineAndTokenPosition> list = new
		// ArrayList<LineAndTokenPosition>();
		// LineAndTokenPosition b = new LineAndTokenPosition();
		// b.setLine(lineNumber);
		// b.setTokenOffset(beginTokenWordNumber);
		// list.add(b);
	...
	// Feature sentenceBeginFeature =
	// annotationType.getFeatureByBaseName("begin");
	...
	// sentenceTypeConstraint);
	//
	// FSIterator<Annotation> filteredIterator =
	// jcas.createFilteredIterator(jcas.getAnnotationIndex().iterator(),
	// beginAndEndAndType);
	...
	//
	//
	// FSMatchConstraint begin = cf.embedConstraint(sentenceBeginFeaturePath,
	// sentenceBeginIntConstraint);
	// FSMatchConstraint end = cf.embedConstraint(sentenceEndFeaturePath,
	// sentenceEndIntConstraint);
	//
